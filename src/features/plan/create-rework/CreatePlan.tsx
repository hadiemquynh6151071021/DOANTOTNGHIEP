...
import PlanWorkItemSection, {
  ITempPlanWorkItem,
} from "./PlanWorkItem/PlanWorkItemSection";
...
export default function CreatePlan() {
  ...
  const [addressCS,setAddressCS] = useState<IAddress>();
  ...
  async function handleCreatePlan(tempWorkItems: ITempPlanWorkItem[]) {
    ...
    try {
      workItems = filteredWorkItems.map((wi) => {
        if (!wi.supervisor) {
          throw new Error(
            "Cần nhập Người giám sát cho hạng mục số" + wi.orderIndex
          );
        }
    ...
    const formBodyData: ICreatePlan = {
      ...
      mdConstructionSite: constructionSiteid, // DO WE REALLY NEED TO PASS A CONSTRUCTION SITE ID???
      mdEmployee: employeeId, // Creator
    ...
    <main className="pt-5 flex flex-col gap-10">
      ...
      <PlanOverviewInfoLayout
        onLoadConstructionSite={
            (constructionSiteid,addressCS)=>{
                setConstructionSiteid(constructionSiteid);
                setAddressCS(addressCS);
            }
        }
        onLoadCostEstimate={(costEstimateId) => {
